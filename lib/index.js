...
function mousemove(e, el, _data) {
  ...
  if (_data.down) {
    ...
    if (el.offsetLeft + _data.width >= window.innerWidth && !movingLeft || el.offsetLeft <= 0 && !movingRight) {
      // do now allow moving outside the window horizontally
    } else {
      el.style.left = _data.draggerOffsetLeft + (e.clientX - _data.initialX) + 'px';
    }
    if (el.offsetTop <= 0 && !movingUp || el.offsetTop + _data.height >= window.innerHeight && !movingDown) {
